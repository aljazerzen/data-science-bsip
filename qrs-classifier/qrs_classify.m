...
function s = normalize_isometric_est(s)
...
function mean_beat = compute_mean_beat(take_first = 100)
  ...
  for j = 1:take_first
    ...
    if beat_idx-before > 0 && beat_idx+after <= length(sig)
      ...
      plot(1:(before + after + 1), beat_sig)
...
function save_beat_distances(beats, sig, mean_beat)
  ...
  for i = 1:size(beats, 1)
    ...
    if beat_idx-before > 0 && beat_idx+after <= length(sig)
      ...
      beat_sig = normalize(sig(beat_idx-before:beat_idx+after), before);
  ...
  csvwrite('beat-distances.csv', distances)
end

function beats = classify(beats, sig, mean_beat, threshold = 0.75)
...
mean_beat = compute_mean_beat(20);

figure; plot(mean_beat);
pause
% save_beat_distances(beats, sig, mean_beat)

% beats = classify(beats, sig, mean_beat);
